function MakeFigure2a_Supplementary(varargin) 

    addpath(genpath([pwd '/fgtools/']));
    cDir = 'A:\Karen\ACC-Cerebellum\'; 
    inpath=[cDir 'data/']; 
    outpath=[cDir ''];
    ...
    [~, ind] = sort(nanmean(ZRateMap_SocPSTH_alll_Object(Purkinje_ID_alll_Object & v3_Object, postt), 2));   
    Purk_psth_neg_all = ZRateMap_SocPSTH_alll_Object(Purkinje_ID_alll_Object & v3_Object, :);
    ...
    AH(7).YDir = 'reverse';  
    text(min(get(gca, 'xlim'))-0.175*diff((get(gca, 'xlim'))), min(get(gca, 'ylim'))-0.1*diff((get(gca, 'ylim'))), 'D', 'FontSize', 14, 'FontWeight', 'Bold');

    axes(AH(10));
    hold off;
    plot(-10, 1, 'color', [155 38 22]./255);
    hold on;
    shadedErrorBar(psth_scale, nanmean(ZRateMap_SocPSTH_alll_Object(v3_Object, :)),...
        nanstd(ZRateMap_SocPSTH_alll_Object(v3_Object, :))./sqrt(sum(v3_Object)), {'color', [155 38 22]./255}, 0.1);
    ...
    postt = psth_scale>0.75 & psth_scale<1.25;
    pre = psth_scale>-2.25 & psth_scale<-1.75;
    [~, ind] = sort(nanmean(ZRateMap_SocPSTH_alll_Object(Purkinje_ID_alll_Object & v4_Object, postt), 2));
    Purk_psth_neg_all = ZRateMap_SocPSTH_alll_Object(Purkinje_ID_alll_Object & v4_Object, :);
    ...
    AH(8).YDir = 'reverse';
    text(min(get(gca, 'xlim'))-0.175*diff((get(gca, 'xlim'))), min(get(gca, 'ylim'))-0.1*diff((get(gca, 'ylim'))), 'E', 'FontSize', 14, 'FontWeight', 'Bold');
  
    axes(AH(11));
    hold off;
    plot(-10, 1, 'color', [231 75 53]./255);
    hold on;
    shadedErrorBar(psth_scale, nanmean(ZRateMap_SocPSTH_alll_Object(v4_Object, :)),...
        nanstd(ZRateMap_SocPSTH_alll_Object(v4_Object, :))./sqrt(sum(v4_Object)), {'color', [231 75 53]./255}, 0.1);
    xlim([-115*0.03 176*0.03]);
    ylim([-2.25 1.75]);
    set(gca, 'TickDir', 'out');
    ...
    postt = psth_scale>0.75 & psth_scale<1.25;
    pre = psth_scale>-2.25 & psth_scale<-1.75;
    [~, ind] = sort(nanmean(ZRateMap_SocPSTH_alll_Object(Purkinje_ID_alll_Object & ~v4_Object & ~v3_Object, postt), 2));
    Purk_psth_neg_all = ZRateMap_SocPSTH_alll_Object(Purkinje_ID_alll_Object & ~v4_Object & ~v3_Object, :);
    P_image = [Purk_psth_neg_all(ind, :); nan(1, size(Purk_psth_neg_all(ind, :), 2))];
    pcolor(psth_scale, 1:size(P_image, 1), P_image);
    shading  flat;  
    hold on;
    ...
    set(gca,'color','none');
    set(gca, 'TickDir', 'out', 'Box', 'off', 'FontSize', 13);
    set(gca, 'YTick', [1.5:4:sum(~all(isnan(P_image')))+4.5], 'YTickLabel', [1:4:sum(~all(isnan(P_image')))+4.5]);
    set(gca,'TickLength', [0.01 0.025]*2.5);   
    LdPos = [-0.02,0.01];  
    AH(9).XAxis.Visible = 'off'; 
    AH(9).YDir = 'reverse';
    % FigLabel(Labels{12}, [0.0001,0.025], 'FontWeight', 'Bold');  
    text(min(get(gca, 'xlim'))-0.175*diff((get(gca, 'xlim'))), min(get(gca, 'ylim'))-0.1*diff((get(gca, 'ylim'))), 'F', 'FontSize', 14, 'FontWeight', 'Bold');
     
    axes(AH(12));
    hold off;
    plot(-10, 1, 'color', [231 75 53]./255);
    hold on;
    shadedErrorBar(psth_scale, nanmean(ZRateMap_SocPSTH_alll_Object(Purkinje_ID_alll_Object & ~v4_Object & ~v3_Object, :)),...
        nanstd(ZRateMap_SocPSTH_alll_Object(Purkinje_ID_alll_Object & ~v4_Object & ~v3_Object, :))./sqrt(sum(Purkinje_ID_alll_Object & ~v4_Object & ~v3_Object)), {'color', [244 155 127]./255}, 0.1);
    xlim([-115*0.03 176*0.03]);
    ylim([-2.25 1.75]);
    set(gca, 'TickDir', 'out');
    ...
    axes(AH(13));
    hold off;
    br = bar(([sum(v3_Object) sum(v4_Object) sum(~v3_Object & ~v4_Object & Purkinje_ID_alll_Object)]/sum(Purkinje_ID_alll_Object))*100);
    br.FaceColor = 'flat';
...
%     FigLabel(Labels{13}, [-0.05, 0.01], 'FontWeight', 'Bold');   
    text(min(get(gca, 'xlim'))-0.275*diff((get(gca, 'xlim'))), max(get(gca, 'ylim'))+0.1*diff((get(gca, 'ylim'))), 'G', 'FontSize', 14, 'FontWeight', 'Bold');

    
%     direc_sv = 'A:\Karen\Figure_1_main_Supplementary';
%     outfile = fullfile(direc_sv, sprintf('SUPPLEMENTARY_FIGURE_1_Purkinje_Object'));
