function MakeFigure2b(varargin) 
    ...
    load([inpath 'PSTH_SocObj_Traces_DCN.mat']); % PSTH_SocObj_Traces.mat
    ...
    fg7 = figure(7);
    clf;
    set(fg7, 'Position', [100 100 900 500]);
...
%     ax12 = axes('Position', [0.35 0.765 0.45 0.1]);
    axes(AH(6));
    hold off;
    shadedErrorBar([-115:176]*0.033, nanmean(theta_sig_z_socc), nanstd(theta_sig_z_socc)./sqrt(size(theta_sig_z_socc, 1)), {'color', [0 102 51]./255}, 0.1); % 
    hold on;
%     shadedErrorBar([-115:176]*0.033, nanmean(theta_sig_z_objj), nanstd(theta_sig_z_objj)./sqrt(size(theta_sig_z_objj, 1)), {'color', [87 87 86]./255}, 0.1); % 
    ...
    xlabel('Social Interaction Onset (sec.)', 'FontSize', 12, 'FontWeight', 'Normal');
    ...
    [~, ind] = sort(nanmean(ZRateMap_SocPSTH_alll_Mutual(~Purkinje_ID_alll_Mutual & v1_Mutual, postt), 2));   
    Purk_psth_neg_all = ZRateMap_SocPSTH_alll_Mutual(~Purkinje_ID_alll_Mutual & v1_Mutual, :);
    pcolor(psth_scale, 1:size(Purk_psth_neg_all, 1), Purk_psth_neg_all(ind, :));
    shading  flat;
    hold on;
    plot([0 0], [0 sum(v1_Mutual)], '--', 'color', [0.7 0.7 0.7], 'LineWidth', 2); 
    xlim([-4 6]);
    ylim([1 sum(v1_Mutual)]);
    cb1 = colorbar('northoutside');
    set(cb1, 'Position', [0.35 0.665 0.05 0.01]);
    caxis([-1 1]);
    cb1.Visible = 'off';
    set(gca,'color','none');
    set(gca, 'TickDir', 'out', 'Box', 'off', 'FontSize', 13);
    set(gca,'TickLength', [0.01 0.025]*2.5);
    set(gca, 'YTick', [1.5:6:sum(v1_Mutual)+.5], 'YTickLabel', [1:6:sum(v1_Mutual)]);
    % text(min(get(gca, 'xlim'))-0.1*diff((get(gca, 'xlim'))), max(get(gca, 'ylim'))+0.125*diff((get(gca, 'ylim'))),...
    %     'a', 'FontSize', 15, 'FontWeight', 'Bold');
    ylabel('DCN (id)', 'FontSize', 12, 'FontWeight', 'Normal');
    ...
    hold on;
    shadedErrorBar(psth_scale, nanmean(ZRateMap_SocPSTH_alll_Mutual(v1_Mutual, :)),...
        nanstd(ZRateMap_SocPSTH_alll_Mutual(v1_Mutual, :))./sqrt(sum(v1_Mutual)), {'color', [0 102 51]./255}, 0.1);
    xlim([-115*0.03 176*0.03]);
    ylim([-0.75 0.75]);
    set(gca, 'TickDir', 'out');
    AH(10).XColor = [0 102 51]./255;
    AH(10).YColor = [0 102 51]./255;
    % axPSTH2.YTick = [];
...
%     xlabel('Social Interaction Onset (sec.)', 'FontSize', 12, 'FontWeight', 'Normal', 'color', 'k');
    ...
    postt = psth_scale>0.75 & psth_scale<1.25;
    pre = psth_scale>-2.25 & psth_scale<-1.75;
    [~, ind] = sort(nanmean(ZRateMap_SocPSTH_alll_Mutual(~Purkinje_ID_alll_Mutual & v2_Mutual, postt), 2));
    Purk_psth_neg_all = ZRateMap_SocPSTH_alll_Mutual(~Purkinje_ID_alll_Mutual & v2_Mutual, :);
    P_image = [Purk_psth_neg_all(ind, :); nan(1, size(Purk_psth_neg_all(ind, :), 2))];
    pcolor(psth_scale, 1:size(P_image, 1), P_image);
    shading  flat; 
    hold on;
    plot([0 0], [0 sum(v2_Mutual)], '--', 'color', [0.7 0.7 0.7], 'LineWidth', 2); 
    xlim([-4 6]);
    cb1 = colorbar('northoutside');  
    set(cb1, 'Position', [0.2 0.5 0.05 0.01]);
    caxis([-1 1]);
    % cb1.Visible = 'off';
    set(gca,'color','none');
    set(gca, 'TickDir', 'out', 'Box', 'off', 'FontSize', 13);
    set(gca, 'YTick', [1.5:3:sum(v2_Mutual)+.5], 'YTickLabel', [1:3:sum(v2_Mutual)]);
    set(gca,'TickLength', [0.01 0.025]*2.5);
    % text(min(get(gca, 'xlim'))-0.1*diff((get(gca, 'xlim'))), max(get(gca, 'ylim'))+0.125*diff((get(gca, 'ylim'))),...
    %     'a', 'FontSize', 15, 'FontWeight', 'Bold');
    ...
    hold on;
    shadedErrorBar(psth_scale, nanmean(ZRateMap_SocPSTH_alll_Mutual(v2_Mutual, :)),...
        nanstd(ZRateMap_SocPSTH_alll_Mutual(v2_Mutual, :))./sqrt(sum(v2_Mutual)), {'color', [46 172 99]./255}, 0.1);
    xlim([-115*0.03 176*0.03]);
    ylim([-0.75 0.75]);
    set(gca, 'TickDir', 'out');
    AH(11).XColor = [46 172 99]./255;
    AH(11).YColor = [46 172 99]./255;
    set(gca,'TickLength', [0.01 0.025]*3);
    plot([0 0], get(gca, 'YLim'), '--', 'color', [0.7 0.7 0.7], 'LineWidth', 1);
    plot(get(gca, 'XLim'), [0 0], '--',  'color', [0.7 0.7 0.7],'LineWidth', 1);
    set(gca, 'TickDir', 'out', 'FontSize', 13);  
    ...
    postt = psth_scale>0.75 & psth_scale<1.25;
    pre = psth_scale>-2.25 & psth_scale<-1.75;
    [~, ind] = sort(nanmean(ZRateMap_SocPSTH_alll_Mutual(~Purkinje_ID_alll_Mutual & ~v2_Mutual & ~v1_Mutual, postt), 2));
    Purk_psth_neg_all = ZRateMap_SocPSTH_alll_Mutual(~Purkinje_ID_alll_Mutual & ~v2_Mutual & ~v1_Mutual, :);
    P_image = [Purk_psth_neg_all(ind, :); nan(1, size(~Purk_psth_neg_all(ind, :), 2))];
    pcolor(psth_scale, 1:size(P_image, 1), P_image);
    shading  flat;  
    hold on;
    plot([0 0], [0 sum(~v2_Mutual & ~v1_Mutual)], '--', 'color', [0.7 0.7 0.7], 'LineWidth', 2); 
    xlim([-4 6]);
    ylim([0 sum(~v2_Mutual & ~v1_Mutual & ~Purkinje_ID_alll_Mutual)]);
    cb1 = colorbar('northoutside');
    ...
    set(gca,'color','none');
    set(gca, 'TickDir', 'out', 'Box', 'off', 'FontSize', 13);
    set(gca, 'YTick', [1.5:20:size(P_image, 1)+0.5], 'YTickLabel', [1:20:size(P_image, 1)]);
    set(gca,'TickLength', [0.01 0.025]*2.5);
    % text(min(get(gca, 'xlim'))-0.1*diff((get(gca, 'xlim'))), max(get(gca, 'ylim'))+0.125*diff((get(gca, 'ylim'))),...
    %     'a', 'FontSize', 15, 'FontWeight', 'Bold');
    % ylabel('Purkinje Cells (#)', 'FontSize', 9, 'FontWeight', 'Normal');
    ...
    plot(-10, 1, 'color', [231 75 53]./255);
    hold on;
    shadedErrorBar(psth_scale, nanmean(ZRateMap_SocPSTH_alll_Mutual(~Purkinje_ID_alll_Mutual & ~v2_Mutual & ~v1_Mutual, :)),...
        nanstd(ZRateMap_SocPSTH_alll_Mutual(~Purkinje_ID_alll_Mutual & ~v2_Mutual & ~v1_Mutual, :))./sqrt(sum(~Purkinje_ID_alll_Mutual & ~v2_Mutual & ~v1_Mutual)), {'color', [155 205 162]./255}, 0.1);
    xlim([-115*0.03 176*0.03]);
    ylim([-0.75 0.75]);
    set(gca, 'TickDir', 'out');
    AH(12).XColor = [155 205 162]./255;
    AH(12).YColor = [155 205 162]./255;
    set(gca,'TickLength', [0.01 0.025]*3);
    plot([0 0], get(gca, 'YLim'), '--', 'color', [0.7 0.7 0.7], 'LineWidth', 1);
    plot(get(gca, 'XLim'), [0 0], '--',  'color', [0.7 0.7 0.7],'LineWidth', 1);
    set(gca, 'TickDir', 'out', 'FontSize', 13);  
%     xlabel('Time (sec.)', 'FontSize', 12, 'FontWeight', 'Normal', 'color', 'k');
    ...
    axes(AH(13));
    hold off;
    br = bar(([sum(v1_Mutual) sum(v2_Mutual) sum(~v1_Mutual & ~v2_Mutual & ~Purkinje_ID_alll_Mutual)]/sum(~Purkinje_ID_alll_Mutual))*100);
    br.FaceColor = 'flat';
    br.CData(1, :) = [0 102 51]./255;
    br.CData(2, :) = [46 172 99]./255;
    br.CData(3, :) = [155 205 162]./255; 
    br.EdgeColor = 'none';
    br.BarWidth = 0.9;
    ylim([0 70]);
    set(gca, 'TickDir', 'out');
